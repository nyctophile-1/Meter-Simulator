...
public sealed class TcpStressService(PushCoordinator push, IHostApplicationLifetime lifetime) : IAsyncDisposable
{
        ...
        try
        {
            ...
            else
            {
                SetState(new TcpStressState("Sending", loop is null
                    ? "Generating and sending one fleet pass. Measure received traffic at the TCP listener."
    ...
    public void Fire()
    {
        lock (_sync)
        {
            ...
            var run = _run;
            _state = _state with { Phase = "Sending", Detail = "Sending the prepared dataset. Measure received traffic at the TCP listener." };
    ...
    private async Task FireAsync(TcpPushRun run)
    {
        ...
        catch (OperationCanceledException) { SetState(State with { Phase = run.InvalidReason is null ? "Stopped" : "Failed", Detail = run.InvalidReason ?? "Run stopped. In-flight delivery may be unconfirmed.", Result = run.LastPass }); }
        catch (Exception ex) { SetState(State with { Phase = "Failed", Detail = ex.Message, Result = run.LastPass }); }
        finally
        {
            await run.DisposeAsync();
            lock (_sync) if (ReferenceEquals(_run, run)) _run = null;
