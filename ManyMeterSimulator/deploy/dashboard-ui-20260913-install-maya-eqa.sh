...
NEEDED=$((APP_BYTES + 2 * 220000000 + 750000000))
...
rollback() {
    ...
    if [ "$SWAPPED" = 1 ]; then
        ...
        FAILED_DATA="$ROOT/data-failed-$STAMP.tar.gz"
        tar -I 'gzip -1' -cf "$FAILED_DATA" -C "$ROOT" data
        gzip -t "$FAILED_DATA"
        tar -dzf "$FAILED_DATA" -C "$ROOT"
        sha256sum -c "$BACKUP/data.tar.gz.sha256"
        test "$(readlink -f "$ROOT/data")" = /opt/maya-sim/data
        find "$ROOT/data" -mindepth 1 -maxdepth 1 -exec rm -rf -- {} +
...
tar -I 'gzip -1' -cf "$BACKUP/data.tar.gz" -C "$ROOT" data
gzip -t "$BACKUP/data.tar.gz"
tar -dzf "$BACKUP/data.tar.gz" -C "$ROOT"
